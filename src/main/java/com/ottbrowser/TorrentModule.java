    ...
    public void startEngine(Promise promise) {
        try {
            ...
            SettingsPack sp = new SettingsPack()
                    .activeDownloads(1)
                    .connectionsLimit(30) // Low-End TV Profile
                    .downloadRateLimit(0)
                    .uploadRateLimit(0);

            // Apply Low-End / Privacy Settings via SWIG to avoid wrapper version mismatch
            settings_pack pack = sp.swig();
            pack.set_bool(settings_pack.bool_types.enable_dht.swigValue(), true);
            pack.set_bool(settings_pack.bool_types.enable_lsd.swigValue(), false);
            pack.set_bool(settings_pack.bool_types.enable_upnp.swigValue(), false);
            pack.set_bool(settings_pack.bool_types.enable_natpmp.swigValue(), false);

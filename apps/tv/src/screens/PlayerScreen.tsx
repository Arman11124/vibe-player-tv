...
export const PlayerScreen: React.FC<{ route: PlayerRoute }> = ({ route }) => {
    ...
    // DIRECT STREAM PLAYBACK (GATEWAY MODE)
    // ─────────────────────────────────────────────────────────────────────────────
    useEffect(() => {
        const resolve = async () => {
            if (initialUrl) {
            ...
            // Gateway Resolution
            if (item) {
                setResolveStatus('Поиск источников...');
                try {
                    const session = await GatewayService.resolveStream(item.id, item.title, item.media_type);
                    if (session && session.streamUrl) {
                        setFinalUrl(session.streamUrl);
                        setResolveStatus(null);
                        setIsResolving(false);
                    } else {
                        setResolveStatus('Потоки не найдены.');
                    }
                } catch (e) {
                    setResolveStatus('Ошибка: ' + e);
                    // Fallback to "Test Stream" is handled inside GatewayService actually, 
                    // but if it fails completely:
                    setIsResolving(false);
                }
            }
        };
    ...
    // Cleanup not needed for simple HTTP stream
        ...
        <SafeAreaView style={styles.container}>
            ...
            {isResolving && (
                <View style={styles.loadingContainer}>
                    ...
                    <Text style={styles.loadingText}>{resolveStatus || 'Инициализация...'}</Text>
                    <Text style={styles.loadingSubText}>Поиск лучших раздач в P2P рое...</Text>
                </View>
            )}

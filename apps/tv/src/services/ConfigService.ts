...
const DEFAULT_CONFIG: AppConfig = {
    maintenance: false,
    parser_url: 'https://tmdb-proxy.dirtyhands-cdn-worker.workers.dev', // Worker Direct (HTTPS)
    plugins: [],
};
...
export const fetchConfig = async (): Promise<AppConfig> => {
    try {
        ...
        // REMOVED: UNCONDITIONAL FORCE (Bypassing block)
        // We now allow Gist to provide the URL, with DEFAULT_CONFIG as a solid fallback
        return combined;
    } catch (error) {
